...
# --- Page Configuration ---
st.set_page_config(import streamlit as stimport streamlit as st
import pandas as pd
import joblib
import time
...
# NEW: Define local image path and load it safely
header_image_path = "assets/header_infographic.jpeg"
...
if model is None or model_columns is None:
    ...
    if predict_button:
        ...
        with col_img:
            # NEW: Define local image path and load it safely
            result_image_path = "assets/house_icon.png"
            if os.path.exists(result_image_path):
                st.image(result_image_path, width=250)
        with col_price:
            formatted_price = f"₹ {prediction[0]:,.0f}"
            st.metric(label="Estimated Property Value", value=formatted_price)
            st.success("This prediction is based on historical market data and the features provided.")
